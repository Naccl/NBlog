		...
		<el-card>
			...
			<el-table :data="commentList" row-key="id" :tree-props="{children: 'replyComments'}" border stripe>
				...
				<el-table-column label="是否公开" width="80">
					<template v-slot="scope">
						<el-switch v-model="scope.row.published" @change=""></el-switch>
					</template>
				</el-table-column>
				<el-table-column label="邮件提醒" width="80">
					<template v-slot="scope">
						<el-switch v-model="scope.row.notice" @change=""></el-switch>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="190">
					<template v-slot="scope">
						<el-button type="primary" icon="el-icon-edit" size="mini" @click="">编辑</el-button>
						<el-popconfirm title="确定删除吗？" icon="el-icon-delete" iconColor="red" @onConfirm="deleteCommentById(scope.row.id)">
